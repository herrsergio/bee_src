...
void compute_local_map(NEURAL_NETWORK_PTR neural_network,
		       ...
		       ROBOT_STATE_PTR  robot_state)
{
  int from_x, to_x, from_y, to_y, x, y, shifted_x, shifted_y, i;
  int local_map_index;
...
main(int argc, char **argv)
{
  ...
  do{
	...
	* (read_end_time.tv_sec - read_begin_time.tv_sec);
      fprintf(stderr, "LASERINT:main:skipping = %f sec\n",
...
#ifdef LASERINT_debug
	fprintf(stderr, "LASERINT:main:sleep period was  = %f sec ***\n",
	     (float)sleep_duration/1000000.0);
#endif
    }
#ifdef LASERINT_debug
...
#ifdef LASERINT_debug
fprintf(stderr, "LASERINT:main:read_script()     -------\n"); 
#endif
